...
def search_client_process(driver):
    now = datetime.now()
    if now.hour < 12:
        name = now.strftime('%d-%b-%Y')
        filename = f'{name} - Periodo de Manha.log'
    else:
        name = now.strftime('%d-%b-%Y')
        filename = f'{name} - Periodo de Tarde.log'

    logging.basicConfig(filename=f'Logs/{filename}',
        filemode='a',
        level=logging.INFO,
        datefmt='%H:%M:%S',
        format='%(asctime)s:%(name)s:%(levelname)s:%(message)s')

    logging.info("STARTING TO SEARCH PROPOSALS")
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logging.info("Error attempting to change frame")
                    print(err)
                    
            except Exception as e:
                logging.info("Error when trying to click on client td")
                print(e)
            ...
            logging.info("Error when attempting to click on search button")
            print(e)

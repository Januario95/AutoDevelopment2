...
destination_folder = config['config']['destination_folder']
...
def get_dataframe_and_send(df, type_):
	...
	filename = f'{path}\\Classified Proposals/{filename}.xlsx'
...
def get_dataframe_then_send(emails, type_):
	...
	for email in emails:
		...
		if type_ == 'NWOW':
			filename = f'{path}/Classified Proposals/Fluxo_nwow.xlsx'
			send_email(receiver=email, 
					   subject='New Ways Of Work',
					   body='This email is for NWOW', 
					   attachment=filename)
		elif type_ == 'CVU Central':
			filename = f'{path}/Classified Proposals/Fluxo_cvu_central.xlsx'
			send_email(receiver='januario.cipriano@standardbank.co.mz', 
				       subject='CVU Central',
				       body='This email is for CVU Central', 
				       attachment=filename)
...
def split_proposal():
	...
	staff.to_excel(f'./Classified Proposals/{filename}', index=False)
	# filename = f'C:/Users/a248433/Documents/SB Mozambique/EDO/Robotics/Bots/Automatizacao do processo de extracao do fluxo/FinalCode/Classified Proposals/{filename}'
...
# df = df[df['Estado'] == 'Proposta Atualizada'][['Nº', 'Colaborador']]
# df.to_excel('Checked.xlsx', index=False)
